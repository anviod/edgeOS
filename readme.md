...
### 1.1 项目背景
随着工业互联网和边缘计算的快速发展，分布式边缘采集网关在工业现场得到了广泛应用。然而，现有的边缘采集系统存在以下问题：
- 单点故障风险高，缺乏有效的冗余机制
- 设备发现和双向数据通信能力不足
- 缺乏统一的调度和协调机制
- 影子设备消息共享机制不完善
...
### 1.2 项目目标
设计并实现一个功能完善的边缘大脑系统，该系统应包含性能强劲的主控节点，负责执行各类调度采集任务、边缘群控算法及节能优化算法。实现N+2冗余架构模式，其中N代表多个分布式边缘采集网关程序，2代表本地主控大脑的双机热备机制（即蜂群模式），包含一个主母皇节点与一个备用母皇节点。该架构需能够协调N个边缘采集网关程序，实现影子设备自动发现、双向数据通信及其他相关核心功能。
