...
func TestNodeHandler_HandleRegister_Success(t *testing.T) {
	...
	h.HandleRegister(nil, buildMsg("edgeCore/nodes/register", payload))
	...
	select {
	case pub := <-publishCh:
		assert.Equal(t, "edgeCore/nodes/n1/response", pub.topic)
...
func TestNodeHandler_HandleRegister_InvalidPayload(t *testing.T) {
	...
	defer cleanup()
	// 非法 JSON，不应 panic
	msg := &mockMessage{topic: "edgeCore/nodes/register", payload: []byte(`{invalid`)}
...
func TestNodeHandler_HandleHeartbeat_UpdatesStatus(t *testing.T) {
	...
	h.HandleHeartbeat(nil, buildMsg("edgeCore/nodes/heartbeat", hb))
...
func TestNodeHandler_HandleHeartbeat_InvalidPayload(t *testing.T) {
	h, cleanup := newTestNodeHandler(t)
	defer cleanup()
	msg := &mockMessage{topic: "edgeCore/nodes/heartbeat", payload: []byte(`bad`)}
...
func TestNodeHandler_HandleUnregister_SetsOffline(t *testing.T) {
	...
	h.HandleUnregister(nil, buildMsg("edgeCore/nodes/unregister", unreg))
...
func TestNodeHandler_HandleUnregister_MissingNodeID(t *testing.T) {
	...
	h.HandleUnregister(nil, buildMsg("edgeCore/nodes/unregister", payload))
}
