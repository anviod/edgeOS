...
// edgeCore 北向 EAN Runtime（ean_bridge.go + DiscoveryPublisher）线格式 fixtures。
// 信封形态对齐 capability.NewEnvelope / CapabilityDescriptorBody / DiscoveryResponseBody。

func TestDecodeCapabilities_edgeCoreNorthboundEnvelope(t *testing.T) {
	// 模拟 edgeCore PublishCapabilityDescriptor → $edgeos/discovery/capability
	payload := []byte(`{
		"header":{
		...
		"body":{
			"capabilities":[
				{"id":"system.diagnostics","agent_id":"edgeCore-node-001","description":"系统诊断","category":"system","timeout_sec":10,"permission":"read"},
				{"id":"ai.protocol_reverse","agent_id":"edgeCore-node-001","description":"AI协议逆向","category":"ai","timeout_sec":60,"permission":"admin"},
				{"id":"modbus_tcp.scan_devices","agent_id":"edgeCore-node-001","description":"扫描设备","category":"device","timeout_sec":30,"permission":"write",
...
func TestDiscovery_IndexNativeAndPurgeV1Bridge(t *testing.T) {
	...
	v1Cap := CapabilityDescriptor{
		ID:       "edgeCore-node-001/dev-1/read-write",
	...
	v1Agent, _ := json.Marshal(AgentDescriptor{
		ID: "edgeCore-node-001", Kind: "edgeCore-gateway", Status: AgentOnline,
		Transport: TransportList{"mqtt"}, HeartbeatIntervalSec: 30,
	})
	dc.HandleAgentOnline(TopicDiscoveryAgent, v1Agent, "v1-bridge")
	require.False(t, dc.HasNativeEANAgent("edgeCore-node-001"))

	// 北向 Runtime 发布原生 Capability → 应清除 V1 合成项
	nativePayload := []byte(`{
		"header":{"message_id":"m1","timestamp":1,"source":"edgeCore-node-001","message_type":"capability_descriptor","version":"2.0"},
		"body":{"capabilities":[
			{"id":"system.diagnostics","agent_id":"edgeCore-node-001","category":"system","timeout_sec":10,"permission":"read"},
			{"id":"bacnet_ip.write_point","agent_id":"edgeCore-node-001","category":"device","timeout_sec":15,"permission":"write"}
		]}
	}`)
	...
	assert.True(t, dc.HasNativeEANCaps("edgeCore-node-001"))

	native, v1 := dc.CountCapabilitiesBySource("edgeCore-node-001")
...
func TestDiscovery_NativeAgentNotOverwrittenByV1Bridge(t *testing.T) {
	...
	// edgeCore 北向 Agent Descriptor（kind=device, transport 为 string, metadata 含非 string）
	nativeAgent := []byte(`{
		"header":{"message_id":"a1","timestamp":1,"source":"edgeCore-node-001","message_type":"agent_descriptor","version":"2.0"},
		"body":{"agent":{
	...
	require.True(t, dc.HasNativeEANAgent("edgeCore-node-001"))
	...
	// V1 Bridge 试图覆盖为 edgeCore-gateway → 必须被拒绝
	v1Agent, _ := json.Marshal(AgentDescriptor{
		ID: "edgeCore-node-001", Kind: "edgeCore-gateway", Status: AgentOnline,
		Transport: TransportList{"mqtt"}, HeartbeatIntervalSec: 30,
	})
...
func TestDiscovery_ResponseIndexesNativeCaps(t *testing.T) {
	...
	v1Raw, _ := json.Marshal(CapabilityDescriptor{
		ID: "edgeCore-node-001/x/read-write", AgentID: "edgeCore-node-001",
	...
	// edgeCore HandleDiscoveryQuery → $edgeos/discovery/response
	resp := []byte(`{
		"header":{
			"message_id":"r1","timestamp":2,"source":"edgeCore-node-001",
			"destination":"edgeos-planner","message_type":"discovery_response",
			"version":"2.0","correlation_id":"q1"
		},
		"body":{
			"agent":{"id":"edgeCore-node-001","kind":"device","version":"2.0.0","status":"online","transport":"mqtt","heartbeat_interval_sec":30},
			"capabilities":[
				{"id":"system.diagnostics","agent_id":"edgeCore-node-001","category":"system","timeout_sec":10,"permission":"read"},
				{"id":"ai.doc_parse","agent_id":"edgeCore-node-001","category":"ai","timeout_sec":60,"permission":"admin"}
			]
		}
	}`)
	...
	require.True(t, dc.HasNativeEANAgent("edgeCore-node-001"))
	require.True(t, dc.HasNativeEANCaps("edgeCore-node-001"))
	_, v1Left := dc.GetCapability("edgeCore-node-001/x/read-write")
...
func TestInvoke_NoV1FallbackForNativeCapability(t *testing.T) {
	...
	dc.HandleAgentOnline(TopicDiscoveryAgent, []byte(`{
		"id":"edgeCore-node-001","kind":"edgeCore-gateway","status":"online","transport":["mqtt"],"heartbeat_interval_sec":30
	}`), "mqtt")
	dc.HandleCapability(TopicDiscoveryCapability, []byte(`{
		"header":{"message_type":"capability_descriptor","source":"edgeCore-node-001","version":"2.0","message_id":"m","timestamp":1},
		"body":{"capabilities":[{"id":"system.diagnostics","agent_id":"edgeCore-node-001","category":"system","timeout_sec":5,"permission":"read"}]}
	}`), "mqtt")
