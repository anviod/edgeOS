---
layout: default
title: edgeCore-EdgeOS MQTT/NATS 通信测试方案
description: edgeCore edgeCore-EdgeOS MQTT/NATS 通信测试方案
---

# edgeCore-EdgeOS MQTT/NATS 通信测试方案

## 概述

本文档详细说明了 edgeCore 边缘采集网关与 edgeOS 节点之间基于 MQTT/NATS 消息中间件的通信测试方案。

## 文档对照表

本文档为 **edgeCore 端测试方案**，与 **EdgeOS 端验证文档** 相互配合。请配合使用：

| edgeCore 测试用例 | 测试内容 | EdgeOS 验证章节 | 说明 |
|---------------|---------|----------------|------|
| 测试 3.1.1 | MQTT 节点注册 | `测试一：节点注册流程` | edgeCore 发送注册请求，EdgeOS 存储并响应 |
| 测试 3.1.2 | 子设备上报 | `测试二：子设备同步流程` | edgeCore 上报设备列表，EdgeOS 同步到 BoltDB |
| **测试 3.1.3** | **点位元数据上报** | `测试二：子设备同步流程` | edgeCore 上报点位定义列表，EdgeOS 存储点位元数据 |
| 测试 3.1.4 | 设备点位值同步 | `测试三：设备点位同步流程` | edgeCore 上报点位全量数据，EdgeOS 存储并响应 |
| 测试 3.1.5 | 实时数据(变化)推送 | `测试四：实时数据推送流程` | edgeCore 推送变化数据，EdgeOS 更新并推送 WebSocket |
| 测试 3.1.6 | 心跳保活 | `测试五：心跳维持流程` | edgeCore 发送心跳，EdgeOS 更新最后活跃时间 |
| 测试 3.1.7 | 控制命令 | `测试七：控制命令下发流程` | EdgeOS 下发命令，edgeCore 执行并响应 |
...
| 操作 | edgeCore 发布 | EdgeOS 发布 | 说明 |
|------|-----------|-------------|------|
| 节点注册 | `edgeCore/nodes/register` | `edgeCore/nodes/{node_id}/response` | |
| 设备上报 | `edgeCore/devices/report` | — | 无需响应 |
| 点位元数据上报 | `edgeCore/points/report` | — | 上报点位定义列表 |
| 设备点位值同步 | `edgeCore/points/{node_id}/{device_id}` | — | 首次/全量同步 |
| 心跳 | `edgeCore/nodes/{node_id}/heartbeat` | — | 无需响应 |
| 实时数据(变化) | `edgeCore/data/{node_id}/{device_id}` | — | 无需响应 |
| 命令下发 | — | `edgeCore/cmd/{node_id}/{command}` | EdgeOS 下发 |
| 命令响应 | `edgeCore/cmd/{node_id}/response` | — | edgeCore 响应 |
| 主动发现 | `edgeCore/discover/request` | `edgeCore/discover/response/{node_id}` | edgeCore 发起 |

### NATS 主题对照

| 操作 | edgeCore 发布 | EdgeOS 发布 | 说明 |
|------|-----------|-------------|------|
| 节点注册 | `edgeCore.nodes.register` | `edgeCore.nodes.{node_id}.response` | 点分隔替代斜杠 |
| 设备上报 | `edgeCore.devices.report` | — | **EdgeOS 必须订阅**（与 MQTT `edgeCore/devices/report` 对称） |
| 点位元数据上报 | `edgeCore.points.report` | — | 上报点位定义列表 |
| 设备点位值同步 | `edgeCore.points.{node_id}.{device_id}` | — | 首次/全量同步 |
| 心跳 | `edgeCore.nodes.{node_id}.heartbeat` | — | |
| 实时数据(变化) | `edgeCore.data.{node_id}.{device_id}` | — | |
| 请求/响应 | — | `edgeCore.req.node.info` / `edgeCore.res.node.info` | NATS 特有 |
...
│                           MQTT/NATS Broker                             │
│                                                                         │
│   edgeCore ──订阅──> edgeCore/cmd/{node_id}/+                                │
│   edgeCore ──发布──> edgeCore/nodes/register         (节点注册请求)           │
│   edgeCore ──发布──> edgeCore/nodes/{node_id}/heartbeat  (心跳)              │
│   edgeCore ──发布──> edgeCore/devices/report           (设备上报)             │
│   edgeCore ──发布──> edgeCore/data/{node_id}/{device_id}  (实时数据)        │
│   edgeCore ──发布──> edgeCore/cmd/{node_id}/response  (命令响应)             │
│                                                                         │
│   edgeOS ──订阅──> edgeCore/nodes/register        (接收节点注册)           │
│   edgeOS ──订阅──> edgeCore/nodes/{node_id}/heartbeat  (接收心跳)        │
│   edgeOS ──订阅──> edgeCore/devices/report        (接收设备上报)           │
│   edgeOS ──订阅──> edgeCore/data/{node_id}/{device_id}  (接收实时数据)    │
│   edgeOS ──发布──> edgeCore/nodes/{node_id}/response  (注册响应)          │
│   edgeOS ──发布──> edgeCore/cmd/{node_id}/{command}  (下发命令)           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| 操作 | 行为 | Topic | 说明 |
|------|------|-------|------|
| 连接时订阅 | **订阅** | `edgeCore/cmd/<node_id>/+` | 接收来自 edgeOS 的所有命令 |
| 节点注册 | **发布** | `edgeCore/nodes/register` | 发送节点注册请求 |
| 心跳 | **发布** | `edgeCore/nodes/<node_id>/heartbeat` | 定期发送心跳保活 |
| 设备上报 | **发布** | `edgeCore/devices/report` | 上报设备列表 |
| 点位元数据上报 | **发布** | `edgeCore/points/report` | 上报点位定义列表 |
| 点位数据同步 | **发布** | `edgeCore/points/<node_id>/<device_id>` | 上报点位全量数据 |
| 实时数据(变化) | **发布** | `edgeCore/data/<node_id>/<device_id>` | 上报变化数据 |
| 命令响应 | **发布** | `edgeCore/cmd/<node_id>/response` | 响应命令执行结果 |

#### edgeOS 角色

| 操作 | 行为 | Topic | 说明 |
|------|------|-------|------|
| 启动时订阅 | **订阅** | `edgeCore/nodes/register` | 接收节点注册请求 |
| 启动时订阅 | **订阅** | `edgeCore/nodes/+` | 接收所有节点级消息 |
| 启动时订阅 | **订阅** | `edgeCore/devices/report` | 接收设备上报 |
| 启动时订阅 | **订阅** | `edgeCore/points/report` | 接收点位元数据上报 |
| 启动时订阅 | **订阅** | `edgeCore/points/+/+` | 接收点位全量同步 |
| 启动时订阅 | **订阅** | `edgeCore/data/+/+` | 接收所有设备数据 |
| 注册响应 | **发布** | `edgeCore/nodes/<node_id>/response` | 返回注册结果和 Token |
| 下发命令 | **发布** | `edgeCore/cmd/<node_id>/<command>` | 下发设备控制命令 |
...
```
edgeCore                              edgeOS                            Broker
 │                                   │                                 │
 │  1. 连接到 Broker                 │                                 │
 │──────────────────────────────────>│                                 │
 │                                   │                                 │
 │  2. 订阅 edgeCore/cmd/{node_id}/+   │                                 │
 │──────────────────────────────────>│                                 │
 │                                   │ 3. 订阅 edgeCore/nodes/register    │
 │                                   │<────────────────────────────────│
 │                                   │                                 │
 │  4. 发布 node_register           │                                 │
 │────────────────────────────────────────────────────────>│
...
**交互说明**:
1. edgeCore 首先连接到 MQTT Broker，并订阅命令接收主题
2. edgeCore 发布 `node_register` 到 `edgeCore/nodes/register`
3. Broker 转发消息给已订阅的 edgeOS
4. edgeOS 解析消息、验证节点信息、存储到 BoltDB
5. edgeOS 发布 `register_response` 到 `edgeCore/nodes/<node_id>/response`
6. edgeCore 接收响应，获取 `access_token` 和 `expires_at`
...
**交互说明**:
1. edgeCore 在节点注册成功后，自动或手动发布设备列表到 `edgeCore/devices/report`
2. edgeOS 接收并解析设备信息，按 `node_id` 关联存储
3. edgeCore 发布点位元数据到 `edgeCore/points/report`，包含所有点位定义（point_id, point_name, data_type, unit, address 等）
4. edgeOS 接收并解析点位元数据，更新 BoltDB 中对应设备的点位定义
5. edgeCore 发布点位值同步到 `edgeCore/points/<node_id>/<device_id>`，包含所有点位当前值
...
**交互说明**:
1. edgeCore 持续采集设备数据，仅当点位值发生变化或达到推送周期时，发布到 `edgeCore/data/<node_id>/<device_id>`
...
**交互说明**:
1. edgeCore 每 30 秒发送一次心跳到 `edgeCore/nodes/<node_id>/heartbeat`
...
**交互说明**:
1. edgeOS 通过 Web UI 或 API 触发设备发现，发布命令到 `edgeCore/cmd/<node_id>/discover`
2. edgeCore 订阅了命令主题，收到消息后执行相应操作
3. 执行完成后，edgeCore 发布 `device_report` 或响应消息
...
**目的**: 验证 edgeCore 节点能够通过 MQTT 成功注册到 edgeOS

##### 3.1.1.1 时序流程图

```
┌──────────────┐     ┌────────────────┐     ┌──────────────┐     ┌──────────────────┐
│    edgeCore     │     │  MQTT Broker   │     │    EdgeOS    │     │     Web UI       │
└──────┬───────┘     └───────┬────────┘     └───────┬──────┘     └────────┬─────────┘
       │                     │                      │                      │
       │  1. Connect &       │                      │                      │
       │     Subscribe       │                      │                      │
...
mosquitto_pub -h 127.0.0.1 -p 1883 \
  ...
  -m '{
    ...
    "body": {
      ...
      "node_name": "edgeCore Gateway Node 001",
      "model": "edgeCore",
      "version": "1.0.0",
      "api_version": "v1",
      "capabilities": ["shadow-sync", "heartbeat", "device-control", "task-execution"],
...
**Topic**: `edgeCore/nodes/<node_id>/response`
...
```bash
# 订阅注册响应
mosquitto_sub -h 127.0.0.1 -p 1883 -t "edgeCore/nodes/edgeCore-node-001/response" -v
```
...
1. 启动 edgeOS 节点
2. 启动 edgeCore 节点
3. edgeCore 发布节点注册消息到 `edgeCore/nodes/register`
4. 验证 edgeCore 接收到 `edgeCore/nodes/<node_id>/response` 响应
...
**预期结果**:
- ✅ EdgeOS 成功接收节点注册消息
- ✅ edgeCore 收到 `register_response` 响应
- ✅ 节点状态显示为 "Online"
- ✅ 节点信息正确显示在 edgeOS 中
...
2. edgeCore 发布设备上报消息到 `edgeCore/devices/report`
...
2. edgeCore 发布点位元数据消息到 `edgeCore/points/report`
...
##### 3.1.4.2 edgeCore 发布消息

**Topic**: `edgeCore/points/<node_id>/<device_id>`
...
```bash
mosquitto_pub -h 127.0.0.1 -p 1883 \
  -t "edgeCore/points/edgeCore-node-001/Room_FC_2014_19" \
  -m '{
    "header": {
...
2. edgeCore 发布点位全量同步到 `edgeCore/points/<node_id>/<device_id>`
...
# 发布实时数据
mosquitto_pub -h 127.0.0.1 -p 1883 \
  -t "edgeCore/data/edgeCore-node-001/Room_FC_2014_19" \
  -m '{
    "header": {
      "message_id": "data-001",
...
2. edgeCore 发布实时数据（变化数据）到 `edgeCore/data/<node_id>/<device_id>`
...
##### 3.1.6.2 edgeCore 发布消息

**Topic**: `edgeCore/nodes/<node_id>/heartbeat`
...
```bash
# 监听心跳消息
mosquitto_sub -h 127.0.0.1 -p 1883 -t "edgeCore/nodes/+/heartbeat" -v

# 发布心跳消息 (每 30 秒执行一次)
mosquitto_pub -h 127.0.0.1 -p 1883 \
  -t "edgeCore/nodes/edgeCore-node-001/heartbeat" \
  -m '{
    "header": {
...
**目的**: 验证 edgeOS 能够下发控制命令到 edgeCore，edgeCore 执行后返回响应
...
└──────┬───────┘     └───────┬────────┘     └───────┬──────┘     └────────┬─────────┘
       │                     │                      │                      │
       │                     │  1. HTTP POST        │                      │
       │                     │  /api/edgeCore/discover │                      │
       │                     │<─────────────────────│                      │
       │                     │                      │  2. Publish          │
       │                     │                      │  discover_command   │
       │                     │                      │─────────────────────>│
       │  3. Subscribe       │                      │                      │
       │  edgeCore/cmd/{node_id} │                      │                      │
       │<────────────────────│                      │                      │
       │  4. Receive          │                      │                      │
       │  discover_command    │                      │                      │
       │<────────────────────│                      │                      │
...
**Topic**: `edgeCore/cmd/<node_id>/<command_type>`
...
**Topic**: `edgeCore/cmd/edgeCore-node-001/discover`
...
**Topic**: `edgeCore/cmd/<node_id>/write`
...
1. 确保节点已成功注册（参考测试 3.1.1）
2. edgeCore 订阅命令主题 `edgeCore/cmd/<node_id>/+`
3. edgeOS 通过 HTTP API 或直接 MQTT 发布命令
4. 验证 edgeCore 接收到命令
5. 验证 edgeCore 执行命令并返回响应
...
##### 3.1.7.5 主动发现 (由 edgeCore 触发)

edgeCore 也可以主动发起设备发现，请求 edgeOS 广播到所有中间件通道：

**edgeCore 发布发现请求**:

**Topic**: `edgeCore/discover/request`
...
**Topic**: `edgeCore/discover/response/<node_id>`
...
> - MQTT: `edgeCore/nodes/register`
> - NATS: `edgeCore.nodes.register`

#### 测试 3.2.1: 节点注册

**目的**: 验证 edgeCore 节点能够通过 NATS 成功注册到 edgeOS

##### 3.2.1.1 时序流程图

```
┌──────────────┐     ┌────────────────┐     ┌────────────────┐     ┌──────────────────┐
│    edgeCore     │     │ NATS Server    │     │    EdgeOS     │     │     Web UI       │
└──────┬───────┘     └───────┬────────┘     └───────┬──────┘     └────────┬─────────┘
       │                     │                      │                      │
       │  1. Connect &       │                      │                      │
       │     Subscribe       │                      │                      │
...
```bash
# 订阅所有消息
nats sub "edgeCore.>" > nats_test.log &

# 发布节点注册消息
nats pub "edgeCore.nodes.register" '{
  "header": {
...
**Subject**: `edgeCore.nodes.<node_id>.response`
...
##### 3.2.2.2 edgeCore 发布消息

**Subject**: `edgeCore.points.<node_id>.<device_id>`
...
```bash
# 发布点位全量同步
nats pub "edgeCore.points.edgeCore-node-001.Room_FC_2014_19" '{
  "header": {
...
##### 3.2.3.2 edgeCore 发布消息

**Subject**: `edgeCore.data.<node_id>.<device_id>`
...
```bash
# 发布实时数据
nats pub "edgeCore.data.edgeCore-node-001.Room_FC_2014_19" '{
  "header": {
...
2. edgeCore 发布实时数据到 `edgeCore.data.<node_id>.<device_id>`
3. 验证 WebSocket 推送 `data_update` 事件
4. 验证 edgeOS 接收到实时数据
5. 验证数据在 Web UI 中正确显示
...
**Subject**: `edgeCore.req.node.info`
...
**Subject**: `edgeCore.res.node.info`
...
```bash
# 使用 NATS 请求/响应模式
nats req "edgeCore.req.node.info" '{
  "header": {
...
# 订阅响应
nats sub "edgeCore.res.node.info"
```

**步骤**:

1. edgeCore 订阅请求主题 `edgeCore.req.node.info`
2. edgeOS 发送请求到 `edgeCore.req.node.info`
3. edgeCore 接收请求并处理
4. edgeCore 发送响应到 `edgeCore.res.node.info`
5. 验证 edgeOS 接收到响应
...
for ((i=1; i<=$RATE*$DURATION; i++)); do
    mosquitto_pub -h 127.0.0.1 -p 1883 -t "edgeCore/data/test/device" -m "{\"value\": $i}" &
    if ((i % $RATE == 0)); then
        sleep 1
    fi
done
...
1. 启动多个 edgeCore 节点（10、50、100、500、1000）
2. 每个节点定期发送心跳和数据
3. 监控 Broker 性能指标
...
1. 启动 edgeOS 和 edgeCore 节点
2. 持续运行 24 小时
3. edgeCore 定期发送心跳和数据
4. 监控内存、CPU 使用情况
5. 监控连接状态
